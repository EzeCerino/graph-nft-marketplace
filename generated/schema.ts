...
export class ItemBougth extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save ItemBougth entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type ItemBougth must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("ItemBougth", id.toString(), this);
    }
  }

  static load(id: string): ItemBougth | null {
    return changetype<ItemBougth | null>(store.get("ItemBougth", id));
  }
